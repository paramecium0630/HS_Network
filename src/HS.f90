program HS_Network
    ...
    real(8) :: xKx1, xKx2, HS_Y, HS_Y_avg, EXP_HS_Y_avg
    ...
    ! dynamic variables
    real(8), allocatable :: f1(:), x1(:), noise_increment(:), fluct(:), fluct_prev(:)!, xt(:,:), ft(:,:)
    ...
    allocate(f1(N), x1(N), noise_increment(N), fluct(N), fluct_prev(N))
...
!dec$endif
    ...
    do k = 1, nreal
        ...
        do ii = -npast, NSTEP_PROTOCOL
            ...
            if (ii == NSTEP_PROTOCOL) exit ! no need to advance state at last protocol step
